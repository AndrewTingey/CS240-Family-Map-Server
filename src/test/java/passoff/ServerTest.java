...
public class ServerTest {
     ...
     * Load
     * Login
     * Model.Model.Event/[eventID]
     */
    @Test
    @DisplayName("Model.Model.Event Valid Test")
    public void testValidEvent(TestInfo testInfo) {
        ...
        try {
            ...
            //This is a valid api call, so the headers sent should be HTTP_OK (200)
            assertHTTP_OK();
            //Checks to make sure eventResult has information for an Model.Model.Event [OR] in the case that there isn't, that there is no error message String
            ...
            //Checks to make sure eventResult has the same information for the "Sheila_Asteroids" Model.Model.Event as it is listed in loadRequest
            assertEquals(loadRequest.getEvent(ASTEROIDS1_EVENT_ID), eventResult.toEvent(), "Model.Model.Event returned does not match event from LoadRequest");
            ...
            //Checks to make sure eventResult has information for an Model.Model.Event [OR] in the case that there isn't, that there is no error message String
            Assertions.assertTrue(eventResult.getMessage() == null || !eventResult.getMessage().toLowerCase().contains("error"), "Result contains an error message");
            //Checks to make sure eventResult has the same information for the "Other_Asteroids" Model.Model.Event as it is listed in loadRequest
            assertEquals(loadRequest.getEvent(ASTEROIDS2_EVENT_ID), eventResult.toEvent(), "Model.Model.Event returned does not match event from LoadRequest");
        } catch (ServerConnectionException e) {
            fail(e.getMessage());
     ...
     * Required API calls:
     * Load
     * (Partial) Model.Model.Event/[eventID]
     */
    @Test
    @DisplayName("Model.Model.Event Bad Auth Token Test")
     ...
     */
    @Test
    @DisplayName("Model.Model.Event Wrong Model.User Test")
     ...
     * HTTP_BAD_REQUEST header. Then makes sure all the Model.Model.Event List is
     * empty. Ends by verifying that an error was reported.
     *
     * @param eventsResult A EventsResult holding an empty Model.Model.Event List and an error message
     */
    ...
    /**
     * First checks to make sure the test that called this function received an
     * HTTP_BAD_REQUEST header. Then makes sure all the data for an Model.Model.Event is
     * empty. Ends by verifying that an error was reported.
     *
     * @param eventResult An EventResult holding an empty Model.Model.Event and an error message
     */
    ...
    private void checkParentsMarriage(EventsResult eventsResult, PersonsResult personsResult, Person person, String relationship, int generationsLeft) {
        ...
        assertNotNull(fatherBirth, relationship + "'s Father's birth Model.Model.Event not included in passoffresult");
        ...
        assertNotNull(fatherMarriage, relationship + "'s Father's marriage Model.Model.Event not included in passoffresult");
        ...
        assertNotNull(motherBirth, relationship + "'s Mother's birth Model.Model.Event not included in passoffresult");
        ...
        assertNotNull(motherMarriage, relationship + "'s Mother's marriage Model.Model.Event not included in passoffresult");
    ...
    private void checkPersonsBirth(EventsResult eventsResult, PersonsResult personsResult, Person person, String relationship, int generationsLeft) {
        ...
        assertNotNull(personBirth, relationship + "'s birth Model.Model.Event not included in passoffresult");
    ...
    private void checkPersonsDeath(EventsResult eventsResult, PersonsResult personsResult, Person person, String relationship, int generationsLeft) {
        ...
        assertNotNull(birth, relationship + "'s birth Model.Model.Event not included in passoffresult");
        int birthYear = birth.getYear();
        Event death = eventsResult.getEvent(person.getPersonID(), DEATH_EVENT);
        assertNotNull(death, relationship + "'s death Model.Model.Event not included in passoffresult");
